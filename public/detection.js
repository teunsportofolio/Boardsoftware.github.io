...
const ACTIVE_COLS = 8;
const START_COL = Math.floor((GRID_COLS - ACTIVE_COLS) / 2);
const END_COL = START_COL + ACTIVE_COLS;

let firstFrameReceived = false;
let sortByGrade = false;
let activeSuggestion = null; // Stores {row, col} for the AI's next recommended hold
let coachMode = false; // The user must turn this on to see suggestions
let livePathBuffers = { // Temporary buffer for AI behavior tracking
    leftHand: [],
    rightHand: [],
    leftFoot: [],
    rightFoot: []
...
coachToggle.addEventListener("click", () => {
    ...
    // UI Feedback
    coachToggle.textContent = coachMode ? "COACH: ON" : "COACH: OFF";
    coachToggle.style.backgroundColor = coachMode ? "#00FFFF" : "var(--accentColor)";
    coachToggle.style.color = coachMode ? "black" : "white";

    // Clear the suggestion if they turn it off mid-climb
    if (!coachMode) activeSuggestion = null;
});
...
// --- BLE ---
    ...
    let lastSentRouteHash = null;
let lastSentSuggestionKey = null;
...
function writeOnCharacteristic(value){
    ...
    function sendGridOverBLE() {
  if (bleServer && bleServer.connected) {
    const pattern = buildPatternStringFromRoute();
    ...
    bleServiceFound.getCharacteristic(ledCharacteristic)
      ...
      .then(() => {
        console.log("Sent grid:", pattern);
        setStatus("Sent: " + pattern);
      })
      .catch(error => {
...
function sendSuggestionIfChanged() {
  if (!bleServer || !bleServer.connected) return;
  if (!coachMode || !activeSuggestion) return;

  const { row, col, forLimb } = activeSuggestion;

  // 🚫 Ignore if outside middle 8 columns
  if (col < START_COL || col >= END_COL) return;

  // ✅ Remap column to 0–7
  const mappedCol = col - START_COL;

  const key = `${row},${mappedCol},${forLimb}`;
  if (key === lastSentSuggestionKey) return;

  lastSentSuggestionKey = key;

  const value = (forLimb && forLimb.includes("Hand")) ? 1 : 2;

  // 🔁 Use mappedCol instead of col
  const pattern = `[${mappedCol},${row},${value}]`;
  ...
  bleServiceFound.getCharacteristic(ledCharacteristic)
    ...
    .then(() => {
      console.log("🤖 Sent NEW suggestion:", pattern);
      setStatus("Suggestion sent");
    })
    .catch(err => console.error("BLE suggestion error:", err));
}
...
function sendRouteIfChanged() {
  ...
  if (currentHash === lastSentRouteHash) return; // no change

  lastSentRouteHash = currentHash;

  const pattern = buildPatternStringFromRoute();
const mappedCol = x - START_COL;
parts.push(`[${mappedCol},${y},${mappedValue}]`);
  ...
  bleServiceFound.getCharacteristic(ledCharacteristic)
    .then(c => c.writeValue(data))
    .then(() => {
      console.log("📡 Sent UPDATED route:", pattern);
      setStatus("Route sent");
    })
    .catch(err => console.error("BLE route send error:", err));
}




function buildPatternStringFromRoute() {
  if (!activeRoute || !activeRoute.grid) return "";

  let parts = [];

  for (let y = START_ROW; y < END_ROW; y++) {
    if (!activeRoute.grid[y]) continue;

    for (let x = 0; x < GRID_COLS; x++) {
      const plannedColor = activeRoute.grid[y][x];
      if (!plannedColor) continue;


      if (plannedColor === "#00FFFF") {
        mappedValue = 3;
      } else {
        mappedValue = 1;
      }
...
// Basic auto-check for Web Bluetooth availability
...
function getRouteHash() {
  ...
  return JSON.stringify(activeRoute.grid);
}

function drawPlannedRoute(Hinv) {
    // 1. Safety check for the transformation matrix
    if (!Hinv) return;
    
    sendRouteIfChanged();
    sendSuggestionIfChanged();
    // 2. DRAW AI COACH SUGGESTION (Runs in both Route and Free mode)
    if (coachMode && activeSuggestion) {
        const { row: r, col: c } = activeSuggestion;
        ...
        // Use the existing highlightCell helper
        // Color coding the suggestion so the climber knows which limb to use
        const suggestionColor = (activeSuggestion.forLimb && activeSuggestion.forLimb.includes("Hand")) 
            ? "#00FFFF"  // Electric Cyan for Hands
            : "#FFA500"; // Bright Orange for Feet

        highlightCell(Hinv, r, c, suggestionColor);
        ctx.shadowColor = suggestionColor;
        
        // Add a white inner border to make it "pop"
        const p1 = warpPoint(Hinv, c / ACTIVE_COLS, r / GRID_ROWS);
        const p2 = warpPoint(Hinv, (c + 1) / ACTIVE_COLS, r / GRID_ROWS);
        const p3 = warpPoint(Hinv, (c + 1) / ACTIVE_COLS, (r + 1) / GRID_ROWS);
        const p4 = warpPoint(Hinv, c / ACTIVE_COLS, (r + 1) / GRID_ROWS);
    ...
    // 3. DRAW PLANNED ROUTE (Only runs if a route is actually loaded)
    if (!activeRoute || !activeRoute.grid) return;
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
        ...
        for (let c = START_COL; c < END_COL; c++) {
            ...
            const isAIHint = coachMode && activeSuggestion && activeSuggestion.row === r && activeSuggestion.col === c;
            ...
            if (isTouched) {
                ...
                // Make the AI suggestion pulse Electric Cyan
                const pulse = (Math.sin(Date.now() / 200) + 1) / 2;
                ctx.fillStyle = "#00FFFF";
                ctx.shadowColor = "#00FFFF";
...
function drawPerspectiveGrid(Hinv) {
    ...
    if (Hinv && typeof Hinv.data32F !== 'undefined') {
        ...
        for (let i = 0; i <= GRID_ROWS; i++) {
            ctx.beginPath();
            for (let j = 0; j <= ACTIVE_COLS; j++) {
                const p = warpPoint(Hinv, j / ACTIVE_COLS, i / GRID_ROWS);
...
function highlightCell(Hinv, row, col, color) {
  const p1 = warpPoint(Hinv, col / ACTIVE_COLS, row / GRID_ROWS);
  const p2 = warpPoint(Hinv, (col + 1) / ACTIVE_COLS, row / GRID_ROWS);
  const p3 = warpPoint(Hinv, (col + 1) / ACTIVE_COLS, (row + 1) / GRID_ROWS);
  const p4 = warpPoint(Hinv, col / ACTIVE_COLS, (row + 1) / GRID_ROWS);
    ...
    if (!filledCells[limb][key]) {
        ...
        if (move) {
            ...
            if (activeLimbs.includes(limb) && 
                ...
                const prevMove = limbMoves.length > 1 ? limbMoves[limbMoves.length - 2] : { row: 18, col: 8 }; 

                filledCells[limb][key].notified = true; 

                // --- CRITICAL FIX START ---
                
                // 1. Always update the bodyState immediately 
                bodyState[limb] = { row: move.row, col: move.col };

                try {
                    if (activeRoute?.id) {
                        await getMoveDifficulty(prevMove, move);
                    }
                    ...
                    if (coachMode) {
                        ...
                        if (isLimbEstablished) {
                            ...
                            statusText.style.color = "var(--accentColor)";
                            console.log(`%c ⏳ Limb Base Established: ${limb} (Move #1). Waiting for next ${limb} move to suggest.`, "color: #FFA500;");
                        }
                    }
                } catch (err) {
...
function onResults(results) {
    ...
    if (results.poseLandmarks) {
        for (let i = 0; i < LIMB_KEYS.length; i++) {
            ...
            // --- NEW: Track hand path for behavior analysis ---
            if (limbName.includes("Hand") || limbName.includes("Foot")) {
                const isCurrentlyMoving = !holdTimers[limbName].start || (performance.now() - holdTimers[limbName].start < 100);
                
                if (isCurrentlyMoving) {
                    // Push to the specific limb's array
                    livePathBuffers[limbName].push({ x: px, y: py });
                    
                    // Keep a 3-second window per limb
                    if (livePathBuffers[limbName].length > 90) livePathBuffers[limbName].shift();
                }
            }
            ...
            if (warped.x >= 0 && warped.x <= 1 && warped.y >= 0 && warped.y <= 1) {
                const col = Math.floor(warped.x * ACTIVE_COLS);
...
async function fetchNextSuggestion(move, anchorForNextMove, limb) {
    ...
    // 2. GLOBAL VIRTUAL FALLBACKS 
    // This handles your requirement: "If other limbs are not yet established, use virtual positions"
    if (!bodyState.leftFoot)  bodyState.leftFoot  = { row: 18, col: 6 };
    if (!bodyState.rightFoot) bodyState.rightFoot = { row: 18, col: 10 };
    if (!bodyState.leftHand)  bodyState.leftHand  = { row: 14, col: 5 };
    if (!bodyState.rightHand) bodyState.rightHand = { row: 14, col: 11 };
    ...
    const targetLimbType = partners[limb];
    
    // Use the coordinates from bodyState (which are either real or virtual)
    const currentPosOfTarget = bodyState[targetLimbType];

    try {
        const response = await fetch('/api/adaptive-twin-live', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                currentHold: { row: move.row, col: move.col }, // Where the moving limb just landed
                previousHold: currentPosOfTarget,             // Where the NEXT limb is currently sitting
                bodyState: bodyState,                         // Full skeleton (Real + Virtual)
                routeId: rId,
                meta: {
                    limb: limb,
                    positions: [...(livePathBuffers[limb] || [])],
                    duration: (livePathBuffers[limb]?.length || 0) * 33
                }
            })
        });
        ...
        if (response.ok) {
            ...
            if (data.chosenHold) {
                activeSuggestion = { 
                    row: data.chosenHold.row, 
                    col: data.chosenHold.col,
                    forLimb: data.nextLimb 
                };
            }

            // Update the UI Box
            const statusText = document.getElementById("coach-status-text");
            const limbLabel = data.nextLimb.replace(/([A-Z])/g, ' $1').trim(); // "leftHand" -> "Left Hand"
            statusText.innerText = `NEXT: ${limbLabel}`;
            ...
            console.log(`%c [PHYSICAL: ${physLabel}] %c [TECHNIQUE: ${chaosLabel}] `, 
                        "color: white; background: #333; font-weight: bold; padding: 2px;", 
                        "color: #00FFFF; background: #222; font-weight: bold; padding: 2px;");

            console.table({
                "Physical Strain": physVal,
                "Movement Chaos": behVal,
                "Total Stress": data.stress
            });

            if (data.chosenHold) {
                console.log(`%c Next Target for ${data.nextLimb}: Row ${data.chosenHold.row}, Col ${data.chosenHold.col}`, "color: #00FFFF; font-weight: bold;");
            }
            
            console.groupEnd();
            livePathBuffers[limb] = []; 
        }
    } catch (err) {
...
function resetGrid() {
  ...
  // 5. Clear the movement path buffers
  Object.keys(livePathBuffers).forEach(limb => {
      livePathBuffers[limb] = [];
  });

  // 6. Reset the Coach UI Elements
  const statusBox = document.getElementById("coach-status-box");
  const statusText = document.getElementById("coach-status-text");
  
  if (statusBox) statusBox.classList.add("hidden");
  if (statusText) {
      statusText.innerText = "INITIALIZING COACH...";
      statusText.style.color = "var(--bgColor)";
  }

  console.log("Grid, BodyState, and Coach UI fully Reset");
}
...
async function openRouteSelector() {
    ...
    // Set the toggle to the current state of the app
    if (coachToggle) {
        coachToggle.checked = coachMode;
        
        // Update the variable whenever the user flips the switch
        coachToggle.onchange = () => {
            coachMode = coachToggle.checked;
            console.log("Coach Mode is now:", coachMode);
        };
    }
    ...
    try {
        ...
        const renderRoutes = () => {
            ...
            filtered.forEach((route, idx) => {
                ...
                item.style.margin = "0 0px 10px 0px";
                ...
                item.querySelector('.climb-info').onclick = () => {
                    const coachToggle = document.getElementById("coachToggle");
                    if (coachToggle) coachMode = coachToggle.checked; // Final sync before closing
